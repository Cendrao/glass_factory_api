defmodule GlassFactoryApi.ApiClient do
  ...
  If no params is passed, no filter is applied.
  """

  @spec get(String.t(), Keyword.t(), map()) :: Tesla.Env.t()
  def get(resource, params \\ [], configuration) do
    config = Configuration.build(configuration)

    with {:ok, result} <- Tesla.get(tesla_client(config), resource, query: params) do
      {:ok, result}
    else
